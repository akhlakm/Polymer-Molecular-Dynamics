...
class Moltemplate(Builder):
    '''Object to perform system structure generation using
    [Moltemplate](https://github.com/jewettaij/moltemplate): A general
    ...
    def _run_emc(self, tmp_file_prefilx : str, output_dir: str, data_fname: str,
                 cleanup: bool):
        # Write the .esh file required to run EMC
        esh_file = os.path.join(output_dir, f'{tmp_file_prefilx}.esh')
        with open(esh_file, 'w') as f:
            f.write('# EMC input file generated by PMD\n')
            f.write('ITEM OPTIONS\n')
            f.write('replace true\n')
            f.write(f'field {self._emc_force_field}\n')
            f.write(f'density {density}\n')
            f.write(f'ntotal {natoms_total}\n')
            f.write('ITEM END\n')
            f.write('\n')
        ...
        try:
            ...
            if not os.path.isfile(f'{tmp_file_prefilx}.pdb'):
                raise RuntimeError("EMC pdb generation failed")
            ...
            os.chdir(previous_dir)
            Pmdlogging.info('Initial configuration generated')

    def _run_moltemplate(self, tmp_file_prefilx : str, output_dir : str,
                    data_fname: str, cleanup: bool) -> None:
        try:
            import moltemplate
        except ImportError:
            raise ImportError('Please install moltemplate using pip.')

        previous_dir = os.getcwd()
        os.chdir(output_dir)

        # Write the .lt file required to run moltemplate
        esh_file = os.path.join(output_dir, f'{tmp_file_prefilx}.lt')
        with open(esh_file, 'w+') as f:
            f.write('# EMC input file generated by PMD\n')
            f.write('ITEM OPTIONS\n')
            f.write('replace true\n')
        ...
        try:
            command = f"moltemplate.sh -atomstyle full" + \
                f"-pdb {tmp_file_prefilx}.pdb {tmp_file_prefilx}.lt"
            subprocess.run(shlex.split(command))

            # confirm file is created
            if not os.path.isfile(f'{tmp_file_prefilx}.data'):
                raise RuntimeError("EMC pdb generation failed")
    ...
    def write_data(self, output_dir: str, smiles: str, density: float,
        ...
        # - Use Moltemplate to build the final Lammps input files.
        self._run_moltemplate()

    def write_functional_form(self, f: TextIOWrapper) -> None:
        if self._force_field.startswith('gaff2'):
            f.write(f'{"pair_style":<15} lj/cut/coul/long 12.0 12.0\n')
            f.write(f'{"pair_modify":<15} mix arithmetic\n')
            f.write(f'{"kspace_style":<15} pppm 1e-4\n')
            f.write(f'{"bond_style":<15} harmonic\n')

...
EMC_EXTS = ('data', 'in', 'params', 'vmd', 'emc.gz', 'pdb.gz', 'psf.gz',
            'cmap')
...
class EMC(Builder):
    ...
    def _run_emc(self, tmp_file_prefilx: str, output_dir: str, data_fname: str, cleanup: bool):
        try:
            ...
            params = {}
            key = None
            pfile = os.path.join(output_dir, f'{tmp_file_prefilx}.params')
            with open(pfile, 'r') as lines:
                for line in lines:
                    parts = line.split()
            ...
            before_coeffs = True
            dfile = os.path.join(output_dir, f'{tmp_file_prefilx}.data')
            with open(dfile, 'r') as lines:
                for line in lines:
                    if line == 'Atoms\n':
                        before_coeffs = False
                    if before_coeffs:
            ...
            with open(os.path.join(output_dir, data_fname), 'w') as f:
            ...
            if cleanup:
                fnames = ['build.emc', f'{tmp_file_prefilx}.esh']
                for ext in EMC_EXTS:
    ...
    def write_data(self, output_dir: str, smiles: str, density: float,
        ...
        with open(f'{tmp_file_prefilx}.esh', 'w') as f:
            ...
            f.write(f'build_dir {output_dir}\n')
            f.write('ITEM END\n')
            f.write('\n')
            f.write('ITEM GROUPS\n')
    ...
    def write_solvent_data(self, output_dir: str, smiles: str,
                           solvent_smiles: str, density: float,
                           natoms_total: int, length: int, nsolvents: int,
                           nchains: int, data_fname: str, cleanup: bool) -> None:
